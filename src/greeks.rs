...
pub fn calculate_greeks(
    ...
    let (delta, theta, bs_price, prob_profit) = if is_call {
        let delta = cdf(d1);
        let theta =
        ...
        let breakeven = k + last_price;
        let d_breakeven =
            ((s / breakeven).ln() + (r - 0.5 * sigma * sigma) * t) / (sigma * t.sqrt());
        let prob_profit = cdf(d_breakeven);
        (delta, theta, bs_price, prob_profit)
    } else {
...
/// Add delta, gamma, theta, vega, bs_price, prob_profit columns to an options DataFrame
/// Requires columns: strike, implied_volatility, dte, option_type, last_price
/// s = current underlying price
...
pub fn add_greeks_to_df(
    ...
    for i in 0..df.height() {
        ...
        let last_price = last_prices.get(i).unwrap_or(0.0);
        ...
        prob_profits.push(greeks.prob_profit);
    }
    ...
    let df_final = df
        ...
        .with_columns([
            ...
            Series::new("prob_profit".into(), prob_profits).lit(),
        ])
        .collect()?;
